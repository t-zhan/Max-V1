...
class Max(PreTrainedModel):
    ...
    def __init__(self, config, is_finetuned=False, **kwargs):
        super().__init__(config)
        self.qwen_model_family = config.qwen_model_family
        qwen_model_cls, qwen_config_cls = _QWEN_FAMILIES[self.qwen_model_family]
        self.qwen_template_meta = TEMPLATE_MAPPING[self.qwen_model_family]
        ...
        self.scheduled_sampling_ratio = config.scheduled_sampling_ratio
        self.post_init()
        ...
        **kwargs,
        ...
        if self.scheduled_sampling_ratio > 0.0:
            ...
            for step in range(seq_len):
                if step == 0:
                    point_inputs = current_points
                elif torch.rand(()).item() > self.scheduled_sampling_ratio:
                    point_inputs = torch.cat((start_point, waypoints[:, :step]), dim=1)
                else:
                    point_inputs = current_points
                ...
                predicted_point = self.reg_head(outputs.hidden_states[-1][:, -1:, :])
                predicted_steps.append(predicted_point)
                current_points = torch.cat((current_points, predicted_point), dim=1)
            predicted_points = torch.cat(predicted_steps, dim=1)
        else:
    ...
    def carla_generate(self, rgb, ego_speed, command_idx):
        command_text = COMMAND_TO_TEXT[int(command_idx)]
        front_concat, back_concat = self._concat_camera_images(rgb)
        # front_concat.save("debug_front_concat.jpg")
        # back_concat.save("debug_back_concat.jpg")
        # raise RuntimeError("debug: images saved")
        
        user_content = (
            f"{B2DVL_IMAGE_DESC}<image><image>"
            "Use information above to answer:\n"
            f"The ego vehicle is driving at the speed of {float(ego_speed):.1f} m/s, "
            f"and it wants to {command_text}. "
            f"{B2DVL_WAYPOINT_QUESTION}"
        )
        encoded = self.inference_template.encode(TemplateInputs.from_dict({
            "messages": [{"role": "user", "content": user_content}],
            "system": MAX_DEFAULT_SYSTEM,
            "images": [front_concat, back_concat],
        }))
        batch = self.inference_template.data_collator([encoded])
        ...
        return pred_waypoints, command_text
    ...
    def from_pretrained(cls, model_name_or_path, *model_args, **kwargs):
        ...
        model.processor = AutoProcessor.from_pretrained(
            model_name_or_path,
            tokenizer_type=model.qwen_model_family,
            padding_side="left",
            backend="torchvision",
        )
        ...
        model_type = f"max_{model.qwen_model_family}"
