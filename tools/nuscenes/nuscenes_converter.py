#!/usr/bin/env python3
"""Generate UniDriveVLA-compatible nuScenes info PKLs without MM dependencies.

This is a lightweight port of UniDriveVLA's ``nuscenes_converter.py``.  It
keeps the original annotation, trajectory, map, camera, and sweep generation
logic while replacing MMCV serialization, progress, and file helpers with
Python's standard library and tqdm.

Derived from UniDriveVLA revision a93c175af893 under Apache-2.0. See
``LICENSES/Apache-2.0.txt`` and ``docs/THIRD_PARTY_NOTICES.md``.

Run from the Max-V1 repository root:
...
The command writes ``nuscenes_infos_train.pkl`` and
``nuscenes_infos_val.pkl`` using pickle protocol 2, matching MMCV's default.
"""
...
from tools.nuscenes.utils.nuscmap_extractor import NuscMapExtractor


def _dump_pickle(data, path):
    """Match ``mmcv.dump(..., file_format='pkl')`` protocol semantics."""
...
def create_nuscenes_infos(root_path,
    ...
    if test:
        ...
        _dump_pickle(data, info_val_path)
...
def _fill_trainval_infos(nusc,
    ...
    for sample in tqdm(nusc.sample, desc="Building nuScenes infos", unit="sample"):
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Generate UniDriveVLA-compatible nuScenes info PKLs "
                    "without MMCV/MMDetection dependencies."
    )
    parser.add_argument(
        "dataset", choices=["nuscenes"], help="dataset name (compatibility argument)"
    )
    parser.add_argument(
        "--root-path",
        default="data/nuscenes",
        help="nuScenes dataset root containing v1.0-trainval, samples, sweeps, and maps",
    )
    parser.add_argument(
        "--canbus",
        default="data/nuscenes",
        help="root containing the nuScenes can_bus directory",
    )
    parser.add_argument(
        "--version",
        choices=["v1.0", "v1.0-mini"],
        default="v1.0",
        help="v1.0 generates trainval files; v1.0-mini generates mini files",
    )
    parser.add_argument(
        "--max-sweeps",
        type=int,
        default=10,
        help="maximum number of previous lidar sweeps per sample",
    )
    parser.add_argument(
        "--out-dir",
        default="data/infos",
        help="output directory for generated info PKLs",
    )
    parser.add_argument(
        "--extra-tag",
        default="nuscenes",
...
def main(argv=None):
    ...
    train_version = (
        args.version if args.version == "v1.0-mini"
        else f"{args.version}-trainval"
    )
    nuscenes_data_prep(
